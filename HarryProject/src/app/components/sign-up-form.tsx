...
export default function SignUpForm() {
  ...
  const validateMobile = (mobile: string): ValidationState => {
    ...
    if (numericMobile.length < 10) return { isValid: false, message: "Mobile number must have at least 10 digits" }
    if (numericMobile.length > 15) return { isValid: false, message: "Mobile number is too long" }
    if (!/^\d+$/.test(numericMobile)) return { isValid: false, message: "Mobile number can only contain digits" }
  ...
  return (
    <div className="w-full bg-gradient-to-br from-yellow-100 to-white flex items-center justify-center p-4 relative overflow-hidden pt-20">
      ...
      <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 0.3 }} className="w-full max-w-md relative">
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-xl overflow-hidden">
          {/* Header */}
          <div className="relative h-32 bg-yellow-400 flex items-center justify-center">
            <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2 }} className="relative text-center">
          ...
          <div className="p-8 space-y-6">
            <div className="space-y-4">
              ...
              <motion.div initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.3 }} className="space-y-2">
                <Label htmlFor="name" className="text-gray-700 flex items-center gap-2"><User size={18} className="text-yellow-500" /> Full Name</Label>
                <Input id="name" name="name" value={formData.name} onChange={handleChange} onBlur={() => handleBlur("name")} placeholder="John Doe" className={cn("pl-4 h-12 transition-all", touched.name && (nameValidation.isValid ? "border-green-500" : "border-red-500"))} />
              ...
              <motion.div initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.4 }} className="space-y-2">
                <Label htmlFor="email" className="text-gray-700 flex items-center gap-2"><Mail size={18} className="text-yellow-500" /> Email</Label>
                <Input id="email" name="email" value={formData.email} onChange={handleChange} onBlur={() => handleBlur("email")} placeholder="johndoe@example.com" className={cn("pl-4 h-12 transition-all", touched.email && (emailValidation.isValid ? "border-green-500" : "border-red-500"))} />
              ...
              <motion.div initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.5 }} className="space-y-2">
                <Label htmlFor="mobile" className="text-gray-700 flex items-center gap-2"><Phone size={18} className="text-yellow-500" /> Mobile Number</Label>
                <Input id="mobile" name="mobile" value={formData.mobile} onChange={handleChange} onBlur={() => handleBlur("mobile")} placeholder="+1 (555) 000-0000" className={cn("pl-4 h-12 transition-all", touched.mobile && (mobileValidation.isValid ? "border-green-500" : "border-red-500"))} />
              ...
              <motion.div initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.6 }} className="space-y-2">
                <Label htmlFor="password" className="text-gray-700 flex items-center gap-2"><Lock size={18} className="text-yellow-500" /> Password</Label>
                <div className="relative">
                  <Input id="password" name="password" type={showPassword ? "text" : "password"} value={formData.password} onChange={handleChange} onBlur={() => handleBlur("password")} placeholder="Enter Password" className={cn("pl-4 h-12 transition-all", touched.password && (passwordValidation.isValid ? "border-green-500" : "border-red-500"))} />
                  <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-yellow-500">{showPassword ? <EyeOff size={20} /> : <Eye size={20} />}</button>
                </div>
              ...
              <motion.div initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.7 }} className="space-y-2">
                <Label htmlFor="confirmPassword" className="text-gray-700 flex items-center gap-2"><Lock size={18} className="text-yellow-500" /> Confirm Password</Label>
                <div className="relative">
                  <Input id="confirmPassword" name="confirmPassword" type={showConfirmPassword ? "text" : "password"} value={formData.confirmPassword} onChange={handleChange} onBlur={() => handleBlur("confirmPassword")} placeholder="Confirm Password" className={cn("pl-4 h-12 transition-all", touched.confirmPassword && (confirmPasswordValidation.isValid ? "border-green-500" : "border-red-500"))} />
                  <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-yellow-500">{showConfirmPassword ? <EyeOff size={20} /> : <Eye size={20} />}</button>
                </div>
            ...
            <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.8 }}>
              <Button type="submit" className="w-full h-12 bg-yellow-400 hover:bg-yellow-500 text-white transition-all">Create Account</Button>
            </motion.div>

            <p className="text-center text-gray-600">
              Already have an account?{" "}
              <a href="/log-in" className="text-yellow-600 hover:text-yellow-700">Sign In</a>
            </p>
          </div>
        </form>
      </motion.div>
